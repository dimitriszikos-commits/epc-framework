...
def run_epc_diagnostics(df, feature_cols, target_col, search_depth=3):
    ...
    for col in feature_cols:
        ...
        # Aligned with EPC Paper Taxonomy
        if is_clinical and is_global:
            classification = "Outcome-Predictive Missingness (Global)"
        elif is_clinical and not is_global:
            classification = "Outcome-Predictive Missingness (Localized)"
        elif not is_clinical and is_global:
            if is_redundant:
                classification = "Structurally Bundled Missingness (Bundled)"
                linked_combo = known_redundancies[col]['Trigger_Combo']
                linked_str = str(linked_combo) 
            else:
                classification = "Structurally Bundled Missingness (Independent)"
        else:
            classification = "Random Non-Informative Missingness"
                
        results.append({
            'Variable': col, 'Missing_SU': round(miss_su, 6), 'IV_Smoothed': round(iv, 4), 
            'LR_Plus': round(lr_plus, 2), 'Odds_Ratio': round(odds_ratio, 4), 'Fisher_P': round(p_val, 4),
            'Classification': classification, 'Linked_Variables': linked_str
        })
    return pd.DataFrame(results), su_dictionary

# ==========================================
# MODULE 5: EPC IMPUTER (ADAPTIVE DIRECTED ROUTING)
# ==========================================
class EPC_Imputer:
    def __init__(self, su_dict=None, cat_cols=None, diagnostic_report=None):
        ...
        self.simple_imputer_num = SimpleImputer(strategy='median')
        self.simple_imputer_cat = SimpleImputer(strategy='most_frequent')
        self.joint_imputer = IterativeImputer(random_state=42, max_iter=10)
        
        # Safety lists to prevent scikit-learn from dropping empty columns
        self.valid_fallback_num = []
        self.valid_fallback_cat = []
        self.valid_protocol = []
        
        self.telemetry = {'Noise_Resolved': 0, 'Protocol_Resolved': 0, 'Prognostic_Underlay_Resolved': 0}

    def fit(self, X_train, y_train=None):
        if self.diagnostic_report is not None:
            for _, row in self.diagnostic_report.iterrows():
                var, cls = row['Variable'], row['Classification']
                if var not in X_train.columns: continue
                    
                if "Random Non-Informative" in cls:
                    self.noise_vars.append(var)
                elif "Structurally Bundled" in cls:
                    self.protocol_vars.append(var)
                else:
                    self.prognostic_vars.append(var)

        fallback_vars = self.noise_vars + self.prognostic_vars
        fallback_num = [v for v in fallback_vars if v not in self.cat_cols]
        fallback_cat = [v for v in fallback_vars if v in self.cat_cols]
        
        # ENFORCEMENT: Only pass columns that have at least one valid observation
        self.valid_fallback_num = [v for v in fallback_num if X_train[v].notna().any()]
        self.valid_fallback_cat = [v for v in fallback_cat if X_train[v].notna().any()]
        self.valid_protocol = [v for v in self.protocol_vars if X_train[v].notna().any()]
        
        if self.valid_fallback_num: self.simple_imputer_num.fit(X_train[self.valid_fallback_num])
        if self.valid_fallback_cat: self.simple_imputer_cat.fit(X_train[self.valid_fallback_cat])
        if self.valid_protocol: self.joint_imputer.fit(X_train[self.valid_protocol])
            
        return self

    def transform(self, X_test, y_test=None):
        X_imp = X_test.copy()
        self.telemetry = {k: 0 for k in self.telemetry}
        
        # 1. Route Noise & Prognostic Underlays
        if self.valid_fallback_num and X_imp[self.valid_fallback_num].isna().any().any():
            self.telemetry['Noise_Resolved'] += X_imp[self.noise_vars].isna().sum().sum() if self.noise_vars else 0
            self.telemetry['Prognostic_Underlay_Resolved'] += X_imp[self.prognostic_vars].isna().sum().sum() if self.prognostic_vars else 0
            X_imp.loc[:, self.valid_fallback_num] = self.simple_imputer_num.transform(X_imp[self.valid_fallback_num])
            
        if self.valid_fallback_cat and X_imp[self.valid_fallback_cat].isna().any().any():
            X_imp.loc[:, self.valid_fallback_cat] = self.simple_imputer_cat.transform(X_imp[self.valid_fallback_cat])

        # 2. Route Protocol Omissions 
        if self.valid_protocol and X_imp[self.valid_protocol].isna().any().any():
            self.telemetry['Protocol_Resolved'] += X_imp[self.valid_protocol].isna().sum().sum()
            X_imp.loc[:, self.valid_protocol] = self.joint_imputer.transform(X_imp[self.valid_protocol])

        # HARD FALLBACK: Plug any remaining edge case NaNs (including 100% empty columns) with 0
        rem_nans = X_imp.columns[X_imp.isna().any()].tolist()
        if rem_nans:
            X_imp[rem_nans] = X_imp[rem_nans].fillna(0)

        print(f"      [Imputer Telemetry] Noise (Simple): {self.telemetry['Noise_Resolved']} | Protocol (MICE): {self.telemetry['Protocol_Resolved']} | Prognostic (Simple Underlay): {self.telemetry['Prognostic_Underlay_Resolved']}")
        return X_imp
...
def prepare_epc(df_train, df_test, target_col, report, su_dict):
    ...
    for _, row in report.iterrows():
        ...
        if classification in ["Outcome-Predictive Missingness (Localized)", "Outcome-Predictive Missingness (Global)"]:
...
if __name__ == "__main__":
    ...
    target_file = input("Enter dataset filename [default: epc_synthetic_validation.csv]: ").strip() or "epc_synthetic_validation.csv"
